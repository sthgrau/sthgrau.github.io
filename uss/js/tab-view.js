	...
	function initTabs(mainContainerID,tabTitles,activeTab,width,height,closeButtonArray,additionalTab)
	{
		...
		for(var no=0;no<tabTitles.length;no++){
			...
			aTab.onmouseout = rolloutTab;
                        if ( tabTitles[no] !==  '\+' ) {
                            console.log(tabTitles[no]);
			    aTab.onclick = tabClick;
			    aTab.id = 'tabTab' + mainContainerID + "_" +  (no + tabView_countTabs[mainContainerID]);
                        }
                        else {
			    aTab.id = 'tabTab' + mainContainerID + "_" +  (7 + no + tabView_countTabs[mainContainerID]);
			    aTab.onclick = function(){createNewTab('dhtmlgoodies_tabView2','xxx','','',true)};
                        }
...
//		dhtmlgoodies_tabObj[parentId].insertBefore(div,dhtmlgoodies_tabObj["dhtmlgoodies_tabView2"].children[dhtmlgoodies_tabObj["dhtmlgoodies_tabView2"].children.length - 1]);
